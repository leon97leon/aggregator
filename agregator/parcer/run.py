...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--date_from', default=date_from, type=str)
    parser.add_argument('-t', '--date_to', default=date_to, type=str)
    ...
    if args.resource == 'news':
        if date_from is not None and date_to is not None:
            ...
            news = p.get_news([RBK(),SRO(),Gaap()], args.date_from, args.date_to)
        else:
        ...
        if args.keywords_type == 'a':
            news = p.check_keywords(news, ['проект'], news_column='Article')
        else:
        ...
        writer = pd.ExcelWriter('theiia_result.xlsx', engine='xlsxwriter')
