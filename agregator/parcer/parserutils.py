...
def check_keywords(news: pd.DataFrame, words: List[str] = None, 
    ...
    def check_article(x):
        try:
            lemm_news = lemmatize(x)
            lemm_sep_news = lemmatize_sep(x)
            ...
            return None
    tqdm.pandas()    
    check_words = news[news_column].progress_apply(check_article)
    filtered_news = news[check_words.notna()]
    filtered_news['Check_word'] = check_words[check_words.notna()]
    return filtered_news


def check_keywords_model(news: List[List[str]], path: str, 
                         news_column: str = 'Article') -> pd.DataFrame:
